/* ─────────────────────────────────────────────
   StockFlow — app.js  v3.0
   Supabase REST API — RLS-aware, no price field
   ─────────────────────────────────────────────*/

'use strict';

// ══════════════════════════════════════════════
// CONFIG  (ค่า hardcode — override ได้จาก Settings)
// ══════════════════════════════════════════════
...
const CONFIG = {
  ...
  SUPABASE_KEY: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZibHVxZ292ZWNyZGFjbW5oemNoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzU3NDQ2MzQsImV4cCI6MjA5MTMyMDYzNH0.G1BpGz-Duv6ANUOvE2Gl_eRCUNT9FO9i69YUAKdJEOg',
};
...
const SB = {
  ready() {
    return !!(CONFIG.SUPABASE_URL && CONFIG.SUPABASE_KEY &&
              CONFIG.SUPABASE_KEY.startsWith('eyJ'));
  },
  ...
  async insertProduct(product) {
    ...
    if (!res.ok) {
      ...
      throw new Error(err.message || `INSERT product ${res.status}`);
    }
  ...
  async updateProduct(id, fields) {
    ...
    const map = { name:'name', category:'category', qty:'qty',
                  lowThreshold:'low_threshold', unit:'unit',
                  barcode:'barcode', note:'note', imageData:'image_data' };
    Object.entries(map).forEach(([k, v]) => { if (k in fields) body[v] = fields[k]; });
    const res = await fetch(
  ...
  async updateQty(id, newQty) { return SB.updateProduct(id, { qty: newQty }); },
  ...
  subscribeProducts(onEvent) {
    ...
    try {
      ...
      ws.onopen = () => ws.send(JSON.stringify({
        topic: 'realtime:public:products', event: 'phx_join',
      ...
      ws.onmessage = (msg) => {
        try {
          ...
          if (data.event === 'postgres_changes' && data.payload?.data) onEvent(data.payload.data);
        } catch(e) {}
      };
      ws.onerror = () => {};
      return ws;
...
const LOCAL = {
  ...
  loadSettings() {
    ...
    // ถ้ามีค่าใน localStorage ให้ override CONFIG
    if (s.url && s.url.includes('supabase.co')) CONFIG.SUPABASE_URL = s.url;
    if (s.key && s.key.startsWith('eyJ'))        CONFIG.SUPABASE_KEY = s.key;
    if (s.low) CONFIG.LOW_STOCK_DEFAULT = parseInt(s.low);
    // populate form fields ถ้ามี
    const f = (id, v) => { const el = document.getElementById(id); if (el && v) el.value = v; };
    f('setting-supabase-url',  s.url || CONFIG.SUPABASE_URL);
    f('setting-supabase-key',  s.key || CONFIG.SUPABASE_KEY);
    f('setting-low-threshold', s.low || String(CONFIG.LOW_STOCK_DEFAULT));
  },
};
...
const DB = {
  async init() {
    ...
    try {
      ...
      SB.subscribeProducts(c => DB._onRealtimeChange(c));
    } catch (err) {
      console.warn('[DB.init] Supabase error:', err.message);
      showSyncStatus('error');
      showToast(`Supabase: ${err.message}`, 'error');
    }
  },

  _onRealtimeChange({ eventType, new: n, old: o }) {
    if (eventType === 'INSERT' && !STATE.products.find(p => p.id === n.id))
      STATE.products.push(SB._fromDb(n));
  ...
  async updateProduct(id, fields) {
    LOCAL.save();
    if (!SB.ready() || !STATE.isOnline) {
      STATE.syncPending.push({ op: 'updateProduct', id, data: fields }); LOCAL.save(); return;
    }
    try { await SB.updateProduct(id, fields); }
    catch { STATE.syncPending.push({ op: 'updateProduct', id, data: fields }); LOCAL.save(); }
  },

  async updateQty(id, qty) {
    LOCAL.save();
    if (!SB.ready() || !STATE.isOnline) {
      STATE.syncPending.push({ op: 'updateQty', id, qty }); LOCAL.save(); return;
    }
    try { await SB.updateQty(id, qty); }
    catch { STATE.syncPending.push({ op: 'updateQty', id, qty }); LOCAL.save(); }
  },

  async deleteProduct(id) {
    LOCAL.save();
    if (!SB.ready() || !STATE.isOnline) {
      STATE.syncPending.push({ op: 'deleteProduct', id }); LOCAL.save(); return;
    }
    try { await SB.deleteProduct(id); }
    catch { STATE.syncPending.push({ op: 'deleteProduct', id }); LOCAL.save(); }
  },

  async addToShopping(pid, qty)     { LOCAL.save(); if (!SB.ready()||!STATE.isOnline) return; try { await SB.addToShoppingList(pid,qty); } catch{} },
  async updateShoppingQty(pid, qty) { LOCAL.save(); if (!SB.ready()||!STATE.isOnline) return; try { await SB.updateShoppingQty(pid,qty); } catch{} },
  async removeFromShopping(pid)     { LOCAL.save(); if (!SB.ready()||!STATE.isOnline) return; try { await SB.removeFromShoppingList(pid); } catch{} },
  async clearShopping()             { LOCAL.save(); if (!SB.ready()||!STATE.isOnline) return; try { await SB.clearShoppingList(); } catch{} },

  async insertHistory(entry) {
    STATE.history.unshift({ ...entry, id: genId(), timestamp: Date.now() });
    LOCAL.save();
    if (!SB.ready() || !STATE.isOnline) return;
    try { await SB.insertHistory(entry); } catch{}
  },

  async clearHistory() {
    LOCAL.save();
    if (!SB.ready() || !STATE.isOnline) return;
    try { await SB.clearHistory(); } catch{}
  },

  async flushPending() {
    if (!SB.ready() || !STATE.syncPending.length) return;
    showSyncStatus('syncing');
    const pending = [...STATE.syncPending];
    STATE.syncPending = []; LOCAL.save();
    let failed = 0;
    for (const op of pending) {
      ...
      } catch { failed++; STATE.syncPending.push(op); LOCAL.save(); }
    }
    showSyncStatus(failed === 0 ? 'synced' : 'error');
    showToast(failed === 0 ? `Sync สำเร็จ ${pending.length} รายการ` : `Sync ล้มเหลว ${failed} รายการ`,
...
function showSyncStatus(status) {
  const dots = [document.getElementById('sync-dot'), document.getElementById('sync-dot-settings')];
  const cls = { synced:'bg-green-400', syncing:'bg-amber-400 animate-pulse', offline:'bg-slate-500', error:'bg-red-400' };
  const title = { synced:'Connected to Supabase', syncing:'Syncing…', offline:'Offline / ไม่ได้ตั้งค่า', error:'Supabase Error' };
  dots.forEach(el => {
    if (!el) return;
    el.className = `w-2 h-2 rounded-full ${cls[status] || cls.offline}`;
    el.title = title[status] || '';
  });
  // อัปเดต label ใน settings ด้วย
  const lbl = document.getElementById('sb-conn-label');
  if (lbl) lbl.textContent = title[status] || '';
}
...
function renderDashboard() {
  ...
  html += `<h3 class="text-sm font-semibold text-slate-300">${STATE.searchQuery ? `ผลการค้นหา "${STATE.searchQuery}" (${products.length})` : `สินค้าทั้งหมด (${products.length})`}</h3>`;
...
function renderProductCard(p) {
  ...
  const lowBadge = low
    ? `<span class="absolute top-2 left-2 bg-red-500/80 backdrop-blur-sm text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full flex items-center gap-0.5"><span class="low-stock-dot w-1.5 h-1.5 bg-white rounded-full"></span>${p.qty===0?'หมด':'ต่ำ'}</span>`
    : '';
  return `
...
function renderHistory() {
  ...
  if (STATE.history.length === 0) {
    ...
    STATE.history.slice(0,50).forEach(h => {
      const isNew = h.type==='new', isAdd = h.delta>0;
...
function startScanner() {
  if (STATE.html5QrCode) { try { STATE.html5QrCode.stop(); } catch{} }
...
async function saveProduct() {
  ...
  try {
    if (STATE.editingProduct) {
      const i = STATE.products.findIndex(p=>p.id===STATE.editingProduct.id);
      if (i >= 0) STATE.products[i] = productData;
...
async function confirmQuickUpdate() {
  ...
  if (i >= 0) { STATE.products[i].qty=newQty; STATE.products[i].updatedAt=Date.now(); }
  closeQuickUpdate();
  await DB.updateQty(p.id, newQty);
  await DB.insertHistory({ productId:p.id, productName:p.name, oldQty, newQty, delta, type:'update' });
  const txt = delta===0?'ไม่เปลี่ยน':delta>0?`+${delta}`:`${delta}`;
  showToast(`${p.name}: ${txt} → ${newQty} ${p.unit||'ชิ้น'}`, 'success');
  renderPage(STATE.currentPage);
}
...
async function toggleShoppingItem(id) {
  ...
  if (idx >= 0) { STATE.shoppingList.splice(idx,1); await DB.removeFromShopping(id); }
  else { STATE.shoppingList.push({ id:product.id, name:product.name, barcode:product.barcode, unit:product.unit, qty:1 }); await DB.addToShopping(id,1); }
  renderPage(STATE.currentPage);
}
...
function exportShoppingPDF() {
  if (!STATE.shoppingList.length) { showToast('ไม่มีรายการที่จะ Export','warning'); return; }
  ...
  doc.autoTable({
    startY:34, head:[['#','ชื่อสินค้า','Barcode','สต็อกปัจจุบัน','จำนวนสั่ง','ซื้อแล้ว']], body:rows,
    styles:{fontSize:9, cellPadding:3}, headStyles:{fillColor:[30,64,175], textColor:255},
...
function clearSearch() {
  ...
  STATE.searchQuery = ''; STATE.filteredProducts = [];
...
function handleImageSelect(event) {
  ...
  img.onload = () => {
    ...
    updateImgPreview(data); URL.revokeObjectURL(url);
  };
  img.src=url;
}

function updateImgPreview(data) {
  const imgEl=document.getElementById('product-img-el'), icon=document.getElementById('cam-icon');
...
function _refreshSettingsUI() {
  const urlDisplay  = document.getElementById('sb-url-display');
  const dotSettings = document.getElementById('sync-dot-settings');
  const connLabel   = document.getElementById('sb-conn-label');
  if (CONFIG.SUPABASE_URL) {
    const project = CONFIG.SUPABASE_URL.replace('https://','').split('.')[0];
    if (urlDisplay) urlDisplay.textContent = project + '.supabase.co';
  } else {
    if (urlDisplay) urlDisplay.textContent = 'ยังไม่ได้ตั้งค่า';
  }
  const dot = document.getElementById('sync-dot');
  const connected = dot?.className?.includes('green');
  if (dotSettings) dotSettings.className = `w-2 h-2 rounded-full ${connected?'bg-green-400':'bg-slate-500'}`;
  if (connLabel) connLabel.textContent = connected ? 'เชื่อมต่อแล้ว' : 'ยังไม่ได้เชื่อมต่อ';
}
...
async function saveSettings() {
  ...
  if (key && !key.startsWith('eyJ')) {
    showToast('Anon Key ต้องขึ้นต้นด้วย eyJ… — ดูได้ที่ Supabase → Settings → API → anon public', 'error');
    return;
  }
  if (url && !url.includes('supabase.co')) {
    showToast('URL ไม่ถูกต้อง ต้องเป็น https://xxxx.supabase.co', 'error');
    return;
  }
  ...
  if (low) CONFIG.LOW_STOCK_DEFAULT = parseInt(low);

  closeSettings();
  showToast('บันทึกแล้ว — กำลังเชื่อมต่อ…', 'success');
  await DB.init();
  renderPage(STATE.currentPage);
}

async function testConnection() {
  if (!SB.ready()) { showToast('กรุณากรอก URL และ Key ก่อน','warning'); return; }
...
function clearLocalCache() {
  openConfirm('ล้าง Local Cache','จะลบ cache ในอุปกรณ์นี้และโหลดใหม่จาก Supabase', async () => {
    ['sf_products','sf_shopping','sf_history','sf_pending'].forEach(k=>localStorage.removeItem(k));
    STATE.products=[]; STATE.shoppingList=[]; STATE.history=[]; STATE.syncPending=[];
    closeSettings();
    showToast('ล้าง cache แล้ว — กำลังโหลดจาก Supabase…','success');
    await DB.init();
    renderPage(STATE.currentPage);
  });
}
...
function clearAllData() {
  openConfirm('ลบข้อมูลทั้งหมด','จะลบสินค้า ประวัติ และ cache ทั้งหมด ไม่สามารถกู้คืนได้', () => {
...
function openNotifications() {
  const low = STATE.products.filter(p=>p.qty<=(p.lowThreshold??CONFIG.LOW_STOCK_DEFAULT));
  if (!low.length) { showToast('ไม่มีการแจ้งเตือน','info'); return; }
  navigate('shopping');
}
